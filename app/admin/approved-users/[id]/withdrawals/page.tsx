...
export default function UserWithdrawalsPage() {
  ...
  const username =
    withdrawals[0]?.user?.name?.toLowerCase().replace(/\s+/g, "") || id;
  ...
  const rejectedAmount = withdrawals.filter((w) => w.status === "rejected").reduce((s, w) => s + w.amount, 0);

  const statusBadge = (status: WithdrawalRecord["status"], updatedAt: string) => {
    const color =
      status === "approved"
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[...Array(3)].map((_, i) => <div key={i} className="h-20 animate-pulse rounded-xl bg-gray-200" />)}
        </div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-gray-800">Withdrawals Log</h1>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {[
          { label: "Approved Withdrawals", value: approvedAmount, bg: "bg-green-500" },
          { label: "Pending Withdrawals",  value: pendingAmount,  bg: "bg-orange-400" },
          { label: "Rejected Withdrawals", value: rejectedAmount, bg: "bg-pink-600" },
        ].map((card) => (
      ...
      {filtered.length === 0 ? (
        <div className="rounded-xl bg-white p-12 text-center shadow-sm">
          ...
          <p className="mt-3 text-sm font-medium text-gray-500">No withdrawals found</p>
        </div>
      ) : (
        <>
          {/* ── Desktop table (md+) ── */}
                  ...
                  {filtered.map((w) => {
                    ...
                    return (
                      <tr key={w.id} className="transition-colors hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3">{statusBadge(w.status, w.updatedAt)}</td>
                        <td className="px-4 py-3">
                          <button
                            onClick={() => navigator.clipboard.writeText(w.trxId)}
              ...
              return (
                <div key={w.id} className="overflow-hidden rounded-xl bg-white shadow-sm">
                  {[
                    ...
                    {
                      label: "Status",
                      value: statusBadge(w.status, w.updatedAt),
                    },
                    {
                      label: "Action",
                      value: (
