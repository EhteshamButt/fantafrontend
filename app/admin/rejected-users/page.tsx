...
export default function RejectedUsersPage() {
  ...
  useEffect(() => {
    adminApi
      .getRejectedUsers()
      .then((res) => setData(res as PaymentRecord[]))
      .catch(console.error)
    ...
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        <div className="rounded-xl bg-white p-12 text-center shadow-sm">
          ...
          <p className="mt-3 text-sm font-medium text-gray-500">No rejected deposits found</p>
        </div>
      ) : (
        <>
          {/* ── Desktop table (md+) ── */}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {filtered.map((item) => {
                    const uname = item.user?.name?.toLowerCase().replace(/\s+/g, "") || "unknown";
                    return (
                      <tr key={item.id} className="transition-colors hover:bg-gray-50">
                        <td className="px-4 py-3">
                          <p className="font-semibold text-indigo-600">{item.packageName}</p>
                          <p className="font-mono text-xs text-gray-500">{item.trxId}</p>
                        </td>
                        ...
                        <td className="whitespace-nowrap px-4 py-3">
                          ...
                          <p className="text-xs font-semibold text-gray-800">
                            {item.amount.toLocaleString("en-PK", { minimumFractionDigits: 2 })} Rs
                          </p>
                        </td>
                        <td className="whitespace-nowrap px-4 py-3">
                          <p className="text-sm text-gray-700">1 Rs = 1.00 PKR</p>
                          <p className="text-xs font-semibold text-gray-800">
                            {item.amount.toLocaleString("en-PK", { minimumFractionDigits: 2 })} PKR
                          </p>
                        </td>
                        <td className="px-4 py-3">
                          <span className="rounded-full border border-red-400 px-3 py-1 text-xs font-semibold text-red-500">
                            Rejected
                          </span>
                          <p className="mt-1 text-xs text-gray-400">{timeAgo(item.updatedAt)}</p>
                        </td>
                        <td className="px-4 py-3">
                          <button
                            onClick={() => {
                              sessionStorage.setItem(`rejected_${item.id}`, JSON.stringify(item));
                              router.push(`/admin/rejected-users/${item.id}`);
                            }}
