...
export default function TodayApprovedDetailPage() {
  ...
  useEffect(() => {
    ...
    const cached = sessionStorage.getItem(`approved_${id}`);
    if (cached) {
      try {
    ...
    // 2. Fallback: fetch via getPaymentById
    adminApi
      .getPaymentById(id)
      .then((p) => {
        sessionStorage.setItem(`approved_${id}`, JSON.stringify(p));
        applyPayment(p);
      })
      .catch(() => {
        // 3. Try today's approved list
        adminApi
          .getTodayApprovedUsers()
          .then((list) => {
            const found = (list as PaymentRecord[]).find((x) => x.id === id);
            if (found) {
              sessionStorage.setItem(`approved_${id}`, JSON.stringify(found));
              applyPayment(found);
            }
          })
          .catch(console.error);
  ...
  if (!payment) {
    return (
      <div className="rounded-xl bg-blue-50 p-8 text-center">
        <p className="text-base font-semibold text-blue-600">Loading deposit details...</p>
        <button
          onClick={() => router.push("/admin/today-approved")}
          className="mt-4 rounded-lg bg-indigo-600 px-5 py-2 text-sm font-semibold text-white hover:bg-indigo-700"
        >
          Go to Approved Deposits
        </button>
      </div>
    );
  }
    ...
    },
    {
      ...
      value: (
        <div className="flex flex-col items-end gap-1">
          <span className="rounded-full border border-green-400 px-3 py-0.5 text-xs font-semibold text-green-600">
            Approved
          </span>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
        ...
        {/* Right: User Deposit Information */}
        <div className="rounded-2xl bg-white shadow-sm">
          <div className="border-b border-gray-100 px-6 py-4">
