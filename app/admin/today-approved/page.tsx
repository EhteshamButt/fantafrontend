    ...
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        <div className="rounded-xl bg-white p-12 text-center shadow-sm">
          ...
          <p className="mt-3 text-sm font-medium text-gray-500">No approved deposits found</p>
        </div>
      ) : (
        <>
          {/* ── Desktop table (md+) ── */}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {filtered.map((item) => {
                    const uname =
                      item.user?.name?.toLowerCase().replace(/\s+/g, "") || "unknown";
                    return (
                      <tr key={item.id} className="transition-colors hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3">
                          <button
                            onClick={() => {
                              sessionStorage.setItem(`approved_${item.id}`, JSON.stringify(item));
                              router.push(`/admin/today-approved/${item.id}`);
                            }}
