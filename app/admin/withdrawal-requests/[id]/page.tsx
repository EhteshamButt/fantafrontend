...
export default function WithdrawalDetailPage() {
  ...
  useEffect(() => {
    ...
    const cached = sessionStorage.getItem(`withdrawal_${id}`);
    if (cached) {
      try {
    ...
    adminApi
      ...
      .then((list) => {
        ...
        if (found) {
          sessionStorage.setItem(`withdrawal_${id}`, JSON.stringify(found));
          ...
          return adminApi.getAllWithdrawals().then((all) => {
            ...
            if (w) {
              sessionStorage.setItem(`withdrawal_${id}`, JSON.stringify(w));
              applyWithdrawal(w);
            }
          });
        }
  ...
  const handleApprove = async () => {
    ...
    setActionMsg("");
    try {
      await adminApi.updateWithdrawalStatus(withdrawal.id, "approved");
      const updated = { ...withdrawal, status: "approved" as const, updatedAt: new Date().toISOString() };
      sessionStorage.setItem(`withdrawal_${id}`, JSON.stringify(updated));
  ...
  if (!withdrawal) {
    return (
      <div className="rounded-xl bg-orange-50 p-8 text-center">
        <p className="text-base font-semibold text-orange-600">Withdrawal data not available</p>
        <button
            ...
            {actionMsg && (
              <p
                className={`text-center text-sm font-medium ${
                  actionMsg.includes("success") || actionMsg === "Approved successfully!"
                    ? "text-green-600"
                    : "text-red-500"
                }`}
              >
